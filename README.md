...
| 접근 granularity | `lines_per_access` 캐시라인을 한 번에 fetch; 시작 주소는 `lines_per_access × 64B` 정렬 블록 기준 (예: N=4면 0~3, 4~7, … 만 가능, 1~4처럼 블록 경계를 넘는 접근 없음) |
| 다중 라인 unroll | `thread_func<LINES>` 템플릿으로 N을 컴파일 타임 상수화; 내부 k-루프가 -O3에서 완전 unroll되어 런타임 분기 없음. `main`에서 `switch`로 한 번만 디스패치 |
| 총 트래픽 | `iters_per_thread × 64 B` (N 무관 고정) — 같은 iters로 granularity만 바꾸면 공정 비교 가능 |
...
```bash
# 무작위 read (기본값: 16코어, 1억회, 4GB 영역, 라인 1개/접근)
./randread_bw [ncores] [iters_per_thread] [hugepages_1gb] [lines_per_access]

# 순차 read (동일 인터페이스, lines_per_access 없음)
./stream_bw [ncores] [iters_per_thread] [hugepages_1gb]

# 예시: 8코어, 5천만회, 4GB 영역, 4라인/접근
numactl -C 0,2,4,6,8,10,12,14 -m 0 ./randread_bw 8 50000000 4 4
numactl -C 0,2,4,6,8,10,12,14 -m 0 ./stream_bw   8 50000000 4
```

- `ncores`: 사용할 코어 수 (기본 16).
- `iters_per_thread`: 스레드당 처리할 총 64B 캐시라인 수 (기본 1억). `randread_bw`는 `UNROLL(16) × lines_per_access` 배수로 내림 조정됨. 총 트래픽은 `iters × 64B`로 `lines_per_access`와 무관하게 고정.
- `hugepages_1gb`: 할당할 1GB hugepage 수 (기본 4). `randread_bw`는 **반드시 2의 거듭제곱**(1, 2, 4, 8, …)이어야 함 (LFSR 마스크 주소 방식 제약). `stream_bw`는 임의의 양수 가능.
- `lines_per_access` (`randread_bw` 전용, 기본 1): 한 번의 무작위 접근에서 fetch할 연속 캐시라인 수. 허용값 `{1, 2, 4, 8, 16}`. 시작 주소는 `lines_per_access × 64B`에 정렬된 블록 경계에서 시작 (예: N=4면 0번, 4번, 8번 … 캐시라인이 시작점, 1~4처럼 경계를 넘는 접근 없음). 접근 횟수 = `iters / lines_per_access`.
...
| rand | `LINES_PER_ACCESS` | 접근당 연속 캐시라인 수 `{1,2,4,8,16}` (rand 모드 전용, 기본 1) |
...
```
ncores=16  iters/thread=100000000  streams=256  region=4 GB  lines/access=1 (64 B)  simd=256 (AVX2)
